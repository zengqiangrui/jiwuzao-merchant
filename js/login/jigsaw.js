...
var failureMsgLock = false
function success(){
	if(!lmx){
		return
	}
	if(failureMsgLock == true){
		return
	}
	failureMsgLock = true;
	setTimeout(function(){
	...
	if(!/^1[0-9]{10}$/.test(example.phone)){
		layer.msg('手机号不符合规则');
		return
	}
if(!/^[a-zA-Z0-9^%&',;=$\x22\(\)\[\]\|\{\}\+\-\*!@#_/<>\.:\?]{8,32}$/.test(example.pwd) || /^[0-9]*[1-9][0-9]*$/.test(example.pwd)){
			layer.msg('密码不符合规则');
			failureMsgLock = true;
			setTimeout(function(){
				failureMsgLock = false;
...
var request = post(true,major+"/userBasic/login",{'phone':example.phone,'pwd':example.pwd,'sha256':sha256(example.phone+'login-kboot')})
...
$(function(){
	jigsaw.init(document.getElementById('slide'))
})

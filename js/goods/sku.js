...
function tableChange(){
		...
		if(b) {
			...
			for(var i = 0; i < totalRow; i++) {
				...
				SKUTableDom += '<tr propvalids=\'' + propvalids + '\' propids=\'' + propIdArr.toString() + '\' propvalnames=\'' + propvalnameArr.join(",") + '\'  propnames=\'' + propNameArr.join(";") + '\' class="sku_table_tr">' + currRowDoms + '<td><input class="setting_sku_price" type="number" value="' + alreadySetSkuPrice + '"/></td><td><input type="number" class="setting_sku_stock" value="' + alreadySetSkuStock + '"/></td></tr>';
			}
			SKUTableDom += "</table>";
		};
...
function getSkuData(){
		...
		$('#skuTable .sku_table_tr').each(function(){
			...
			contArr.spec.push({'specClass':specs,'specPrice':price,'specInventory':inventory});
		});

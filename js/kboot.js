...
function storageSet(key, value, interval, expire) {
	if (interval && expire) {
		var endTime = dateAdd(new Date(), interval, expire).getTime()
	} else {
		var endTime = dateAdd(new Date(), 'y', 100).getTime()
	}
	var obj = {
		data: value,
		endTime: endTime
...
function uploadImg(style, success) {
		...
		handleRequest(request, function(result) {
			...
			$.ajax({
				...
				success: function() {
					...
					var styleUrl;
					var square = url + '?imageView2/1/w/200/h/200/q/75|imageslim'
					if (style) {
						styleUrl = url + "?imageView2/2/w/" + style.width + "/h/" + style.height + "/q/75|imageslim"
					} else {
						styleUrl = null
					}
